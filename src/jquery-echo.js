...
(function($){
                ...
                if (ele !== "undefined"){
                    ...
                    if(newMsg[ele].toString().indexOf("function")>=0){
                        ...
                        if(newMsg[ele].toString()=='[object Object]'){
                            level = level + 1;
                            if (level<=maxlevel) {
                                txtEdited += this.evaluate(newMsg[ele], level, maxlevel);
                            }
                        }else{
    ...
    Echo.prototype.createBox = function () {
        ...
        */
        ...
        $(_this.el).on("dblclick", function(e){
            _this.destroy();
        });

        $('a', _this.el).on("click", function(e){
            _this.destroy();
        });
        ...
        if (_this.settings.slide) {
            ...
            _this.$el.slideDown(_this.settings.speed);
        };
            ...
            for (var i = 0; i < arrId.length; i++) {
                ...
                if ($elem!=null) {
                    if($elem.attr('data-type')===_this.settings.type){
                        var $span = $('<span>'+_this.settings.msg+'</span>');
                         if ($span.text()==$elem.text().substring(0, $elem.text().length-1)) {
                            _this.isRepeat=1;
                            $span = null;
                            break;
                        }
    ...
    Echo.prototype.destroy = function () {
        ...
        if (_this.destroyed===0) {
            ...
            if (_this.settings.slide === true) {
                _this.$el.slideUp((_this.settings.speed+100), function(){
                    _this.$el.remove();
                    _this.destroyed = 1;
    ...
    Echo.prototype.evaluate = function (msg, level, maxlevel){
...
function echo(message, lifetime) {
   ...
   return $.fn.Echo({msg : message, type : 'echo', lifetime : lifetime});
}
...
function notice(message, lifetime) {
    if (!lifetime) {
        lifetime = 30000;
    }
    $.fn.Echo({msg : message, type : 'notice', lifetime : lifetime, slide : true});
}
